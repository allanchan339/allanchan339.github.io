...
tags: [ai, review, diffusion, generation, super-resolution]
categories: [research]
---

# Introduction

It is a pioneer for super resolution (SR) task via diffusion model. For the detail of diffusion model, please refer to [DDPM](/research/2022/12/20/Review-DDPM.html) or [DDPM-Bayes](/research/2023/02/21/DDPM-Bayes.html).
...
\begin{aligned}
    ...
    &\text{repeat} \\
    &\quad \text{Sample} (img_{lr}, img_{hr}) \sim I_{data} \\
    &\quad \bar{img}_{hr} = D(img_{lr}); \\
    &\quad \text{take gradient step on }  \nabla_\theta\Vert img_{hr} - \bar{img}_{hr} \Vert \\
    &\text{until converged }
\end{aligned}
$$
...
\begin{aligned}
    ...
    & \text{repeat} \\
    &\quad \text{Sample} (img_{lr}, img_{hr}) \sim I_{data} \\
    &\quad \text{Upsample} img_{lr} \to up(img_{lr}) \text{by BICUBIC} \\
    &\quad R_{lr} = img_{hr} - up(img_{lr}) = x_0\\
    &\quad x_e = D(img_{lr}) \\
    &\quad x_t = \sqrt{\bar{a}_t}x_0  + \sqrt{1 - \bar{a}_t}\epsilon \\
...
\begin{aligned}
    ...
    &\quad \text{Sample } t = \begin{cases}
            N(0,I) if t > 1 \\
            0
        \end{cases} \\
    &\quad x_{t-1} = q(x_{t-1}\vert x_t, \hat{x_0}), \text{details in DDPM Bayes}\\
    &\text{end for} \\
