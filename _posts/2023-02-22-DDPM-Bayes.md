...
Recall the Gaussian density function \\(f(x)=\frac{1}{\sqrt{2 \pi}\sigma_f}\exp\!\left(-\frac{1}{2}\frac{(x-\mu_f)^2}{\sigma_f^2}\right)=\boldsymbol{N}(\mu_f,\sigma_f^2)\\).

We focus on the coefficient of \\(\boldsymbol{x}_{t-1}^2\\). Since this term is quadratic, the resulting form remains Gaussian. We obtain the quadratic form by [completing the square](https://www.mathsisfun.com/algebra/completing-square.html):
...
x_0 &= \frac{1}{\sqrt{\bar{\alpha_t}}}\big(x_t-\sqrt{1-\bar{\alpha}_t}\boldsymbol{\epsilon}\big) \\
...
\begin{align}
    ...
    x_{t-1} &= \sqrt{\bar{\alpha}_{t-1}} x_{0} + \sqrt{1-\bar{\alpha}_{t-1}}\boldsymbol{\bar{\epsilon}}_{t-2} \sim \mathcal{N}\big(\sqrt{\bar{\alpha}_{t-1}} x_{0}, (\sqrt{1-\bar{\alpha}_{t-1}})^2I\big)\\
    ...
    &= \sqrt{\bar{\alpha_t}} x_{0} + \sqrt{1-\bar{\alpha_t}}\boldsymbol{\bar{\epsilon}_{t-1}} \space \text{;where } \boldsymbol{\bar{\epsilon}_{t-1}} \text{ is merging result of } \epsilon_1, \epsilon_2, \dots, \epsilon_{t-1} \sim \mathcal{N}(0,1) \\
    &= \mathcal{N}\big(x_t;\sqrt{\bar{\alpha}_{t}} x_{0}, (\sqrt{1-\bar{\alpha}_{t}})^2I\big) \nonumber
\end{align}
$$
...
\begin{align}
    ...
    &=  \sqrt{\bar{\alpha}_{t-1}} x_{0} + \frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1})}{1-\bar{\alpha}_{t}} x_t - \frac{\sqrt{\alpha_t\bar{\alpha_t}}(1-\bar{\alpha}_{t-1})}{1-\bar{\alpha_t}}x_0 \nonumber\\
...
Finally, we have a closed form for reversed process:

$$
\begin{aligned}
    p(x_{t-1}\vert x_t, x_0) &= \mathcal{N}(x_{t-1}; \mu(x_{t-1}\vert x_t, x_0), \Sigma(x_{t-1}\vert x_t, x_0)) \nonumber \\
    &= \mathcal{N}(x_{t-1};\frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1})}{(1-\bar{\alpha_t})}\boldsymbol{x}_t + \frac{\sqrt{\bar{\alpha}_{t-1}}\beta_t}{(1-\bar{\alpha_t})}\boldsymbol{x}_0, \frac{(1-\bar{\alpha}_{t-1})\beta_t}{(1-\bar{\alpha_t})}I) \\
    p(\boldsymbol{x}_{t-1}|\boldsymbol{x}_t) \approx p(\boldsymbol{x}_{t-1}|\boldsymbol{x}_t, \boldsymbol{x}_0=\bar{\boldsymbol{\mu}}(\boldsymbol{x}_t)) &= \mathcal{N}\left(\boldsymbol{x}_{t-1}; \frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1})}{(1-\bar{\alpha_t})}\boldsymbol{x}_t + \frac{\sqrt{\bar{\alpha}_{t-1}}\beta_t}{(1-\bar{\alpha_t})}\bar{\boldsymbol{\mu}}(\boldsymbol{x}_t),\frac{(1-\bar{\alpha}_{t-1})\beta_t}{(1-\bar{\alpha_t})} \boldsymbol{I}\right) \\
